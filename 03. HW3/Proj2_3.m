...
[Ia, Ia_bin] = extract_digits(Ia_orig,AVG_FILTER_SIZE,DIGIT_SIDE);

% [Ia, Ia_bin] = extract_digits(Ia_orig,AVG_FILTER_SIZE,BIN);
% % Ib = Templates{8};
...
for i=1:length(Templates)
    ...
    [Ib, Ib_bin] = extract_digits(Ib_orig,AVG_FILTER_SIZE,DIGIT_SIDE);
%     [Ib, Ib_bin]  = extract_digits(Ib_orig,AVG_FILTER_SIZE,BIN);
...
%     db = Templates{i};
    ...
    pairsA = zeros(1,2);
    pairsB = zeros(1,2);
    select_matches = zeros(2,1);
    
    xb_no_shift = fb(1,matches(2,:));
    j = 1;
    k = 1;
    for a=1:length(xa)
        dummy_a = round([ya(a) xa(a)]);
        dummy_b = round([yb(a) xb_no_shift(a)]);
        
        if (Ia_bin{1}(dummy_a(1),dummy_a(2)))
            pairsA(j,:) =  [xa(a) ya(a)];
            j = j+1;
            select_matches;
        end
        
        if (Ib_bin{1}(dummy_b(1),dummy_b(2)))
            pairsB(k,:) =  [xb(a) yb(a)];
            k = k+1;
        end
    end
